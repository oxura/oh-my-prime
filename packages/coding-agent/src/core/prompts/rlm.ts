...
export function buildRlmPrompt(options: RlmPromptOptions): string {
	...
	if (allowRecursion && hasIpython) {
		parts.push(
			"",
			"A callable `rlm` is already in your global namespace. `await rlm('sub-task')` spawns a child and returns immediately after task admission with `rlm_child_id`, `name`, `session_dir`, `model`, `cwd`, effective `effort`, and the enforced `capabilities`; it never waits for or returns the child's answer.",
			...
			"Every child is OS-sandboxed by a capability manifest. The default grants read/write only under the child cwd, denies outbound network and secrets, limits the process tree to 64 processes, and stops it after 20 minutes. Narrow it with `capabilities={'filesystem': {'read': ['src/**'], 'write': ['src/auth/**', 'tests/auth/**']}, 'network': False, 'secrets': {'allow': []}, 'process': {'memory_bytes': '4gb', 'wall_time_ms': '20m'}}`; nested children inherit their parent's ceiling and cannot widen it.",
