...
- Added Evolution Lab proof-backed memory candidates with verifier-attested evidence, separate knowledge/hypothesis/error/temporary categories, dependency invalidation, confidence decay, and an optimistic event ledger.
- Added Evolution Lab replay suites, isolated baseline/candidate workspaces, shadow harnesses, regression-gated promotion, provenance-rich active memory, and stale-safe automatic rollback.
- Added a durable Flow Runtime with dependency-aware task graphs, typed content-addressed artifacts, provider-measured token budgets, orphan-safe retries, resource exclusion, quorum joins, cancellation, and semantic RLM routing.
- Added fail-closed per-child capability manifests with non-widening filesystem, network, secret, CPU, memory, wall-time, and process limits, host-private session controls, private skill state, credential isolation, daemon-safe attestation, and standalone sandbox assets.
